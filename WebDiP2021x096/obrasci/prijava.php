...
if (isset($_POST['prijavi_se'])) {
    ...
    if (count($rezultat) == 0) {
        ...
        if ($aktiviran == 0) {
            ...
            $pokusaji_prijave = $rezultat[0]['pokusaji_prijave'];
            ...
            $broj_preostalih_pokusaja = $dozvoljene_prijave - $pokusaji_prijave;

            if ($broj_preostalih_pokusaja === 0) {
                $greske['lozinka'] = "Kriva lozinka! Previše puta ste upisali krivu lozinku! Vaš račun je blokiran";
    ...
    if (count($greske) == 0) {
        ...
        Sesija::kreirajKorisnika($korisnik['korisnik_id'], $korisnik['uloga_id'], $korisnik['korisnicko_ime']);
        $baza = new Baza();
        $baza->spojiDB();
